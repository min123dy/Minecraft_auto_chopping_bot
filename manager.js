...
function main() {
    ...
    function spawnNextBot() {
        if (currentBotIndex > config.BOT_COUNT) {
            ...
            setTimeout(() => {
                console.log("\n[메니저] 모든 개별 봇 프로세스에 명령 하달!");
                runningProcesses.forEach((child) => {
                    if (child.connected) child.send("mine_logs");
                });
            }, 5000);
            return;
        }
        ...
        child.on("exit", (code) => {
            console.log(`[메니저 경고] ${botName} 프로세스 종료 (코드: ${code})`);
        });
...
process.on("exit", () => { runningProcesses.forEach(child => child.kill()); });
main();